...
exports.getByPost = (req, res) => {
    ...
    const queryCheckForLikesCount = `SELECT COUNT(*) AS likes_count FROM likes
    WHERE post_id = ${post_id}`
    sql.query(queryCheckForLikesCount, (err, result) => {
        if (err) {
            res.send({ success: false, msg: 'Error on queryCheckForLikesCount' })
        } else {
